...
// PATCH /api/videos/:id/assign-affiliate — aggregator/admin phân video vào tài khoản affiliate.
export async function PATCH(
  ...
  try {
    ...
    const update: VideoUpdate = {
      ...
      status: affiliateId ? "assigned" : "submitted",
    };
    ...
    const { data, error } = await supabase
      ...
      .update(update)
      .eq("id", id)
      .select("id, assigned_affiliate_account_id, status")
      .maybeSingle();
