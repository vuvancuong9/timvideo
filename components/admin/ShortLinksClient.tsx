...
export function ShortLinksClient({ videos }: { videos: VideoRow[] }) {
  const router = useRouter();
  const [links, setLinks] = useState<Record<string, string>>(() =>
    Object.fromEntries(videos.map((v) => [v.id, v.short_link ?? ""])),
  );
  ...
  async function save(id: string) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/videos/${id}/short-link`, {
        method: "PATCH",
