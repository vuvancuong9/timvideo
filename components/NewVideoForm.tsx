...
import { detectSource } from "@/lib/url/canonical";
import { VIDEO_SOURCE_LABELS } from "@/lib/constants";
...
export function NewVideoForm({ categories }: { categories: Category[] }) {
  ...
  const source = useMemo(
    () => (videoUrl.trim() ? detectSource(videoUrl) : null),
    [videoUrl],
  );
  ...
  // Debounce 500ms kiểm tra trùng link video.
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(
          `/api/videos/check-duplicate?url=${encodeURIComponent(url)}`,
        );
        const json = await res.json();
        lastChecked.current = url;
        setDuplicate(Boolean(json.duplicate));
      } catch {
        // bỏ qua lỗi mạng tạm thời; server vẫn chặn khi submit
      } finally {
        setChecking(false);
      }
    }, 500);
  ...
  async function uploadToDrive(f: File): Promise<DriveMeta> {
    ...
    if (!completeRes.ok) {
      throw new Error(completeJson.error || "Không hoàn tất được upload Drive");
    }
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setError(null);
    if (duplicate) {
      setError("Video này đã có người nhập rồi.");
      return;
    }
    setSubmitting(true);
    try {
      ...
      if (file) {
        drive = await uploadToDrive(file);
      }
  ...
  return (
    <form onSubmit={onSubmit} className="max-w-2xl space-y-5">
      ...
      <Field label="Link video tìm được" required>
        <input
          type="url"
          required
          value={videoUrl}
          onChange={(e) => setVideoUrl(e.target.value)}
          placeholder="Link Facebook / TikTok / YouTube / khác"
          className={inputClass}
        />
        ...
        {duplicate && (
          <p className="mt-2 rounded-lg bg-red-50 px-3 py-2 text-sm font-medium text-red-700">
            ⚠️ Video này đã có người nhập rồi.
          </p>
        )}
      </Field>
      ...
      <Field label="Upload video lên Google Drive">
        <input
          type="file"
          accept="video/*"
        ...
        {progress !== null && (
          <div className="mt-2">
            ...
            <p className="mt-1 text-xs text-gray-500">Đang upload {progress}%</p>
          </div>
        )}
      </Field>
